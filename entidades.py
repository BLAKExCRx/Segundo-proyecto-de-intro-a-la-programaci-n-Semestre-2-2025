...
class Terreno:
    ...
    def _cargar_imagen(self):
        """Intenta cargar la imagen del terreno"""
        try:
            nombre_archivo = f'assets/{self.tipo}.png'
            self.imagen = pygame.image.load(nombre_archivo)
            self.imagen = pygame.transform.scale(self.imagen, (TAM_CELDA, TAM_CELDA))
        except pygame.error:
            print(f"No se pudo cargar imagen: assets/{self.tipo}.png")
            self.imagen = None
    ...
    def dibujar(self, screen, fila, col):
        ...
        if self.imagen:
            ...
            # Fallback a color si no hay imagen
...
class Camino(Terreno):
    def __init__(self):
        super().__init__('suelo')  # Usa 'suelo.png'
...
class Entidad:
    ...
    def mover(self, mapa, df, dc, modo=None):
...
class Jugador(Entidad):
    def __init__(self, fila, col):
        ...
        self.recuperacion = 0.05  # Regeneración MUY lenta (era 0.15)
        self.consumo_correr = 2

    def puede_acceder(self, terreno):
        return terreno.es_accesible_jugador()

    def correr(self, mapa, df, dc, modo=None):
        """Corre consumiendo energía - permite movimiento más rápido"""
    ...
    def actualizar_energia(self):
        """Regenera energía lentamente cuando no se mueve"""
